...
function getServiceConfig() {
  const supaUrl = readEnv('NEXT_PUBLIC_SUPABASE_URL') || readEnv('SUPABASE_URL');
...
function getSupabaseAdminClientInstance(supaUrl, serviceKey) {
  const cacheKey = `${supaUrl}::${serviceKey}`;
  if (!cachedAdminClient || cachedAdminKey !== cacheKey) {
...
function buildOnConflictValue(columns) {
  if (!columns) return undefined;
  if (Array.isArray(columns)) return columns.join(',');
  return columns;
}

async function upsertRowsOrThrow(client, table, rows, { onConflict } = {}) {
  if (!client || !Array.isArray(rows) || !rows.length) return;
  const options = {};
  const conflict = buildOnConflictValue(onConflict);
  if (conflict) options.onConflict = conflict;
  const { error } = await client.from(table).upsert(rows, options);
  if (error) {
    throw new HttpError(error.status ?? 500, 'Supabase upsert error', error.message ?? error.details ?? error);
  }
}
  ...
  if (typeof raw === 'string') {
    ...
    try {
      ...
      try {
        ...
        console.warn('[anon-children] Recovered from malformed JSON');
        return parsed;
      } catch {
        console.warn('[anon-children] Failed to recover malformed JSON');
        return { summary: trimmed.slice(0, 800) };
      }
    }
  }
  return {};
  ...
  try {
    ...
    if (action === 'create') {
      ...
      if (measurements.length) {
        await upsertRowsOrThrow(supabaseAdmin, 'growth_measurements', measurements, { onConflict: ['child_id', 'month'] });
      }
      const teeth = withChildId(buildTeethRecords(body.growthTeeth), childId);
      if (teeth.length) {
        await upsertRowsOrThrow(supabaseAdmin, 'growth_teeth', teeth, { onConflict: ['child_id', 'month'] });
      }
