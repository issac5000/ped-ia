...
async function fetchParentUpdates(supaUrl, headers, profileId, limit = 12) {
  ...
  const rows = await supabaseRequest(
    `${supaUrl}/rest/v1/parent_updates?select=id,update_type,update_content,parent_comment,ai_commentaire,comment_origin,created_at&profile_id=eq.${encodeURIComponent(profileId)}&order=created_at.desc&limit=${safeLimit}`,
    { headers }
  );
...
export async function processAnonParentUpdatesRequest(body) {
  try {
    ...
    if (action === 'list') {
      ...
      const [profileRow, updates] = await Promise.all([
