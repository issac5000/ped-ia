...
export async function processAnonCommunityRequest(body) {
  try {
    ...
    if (action === 'list') {
      ...
      let profiles = [];
      let hasChildCount = true;
      let showColumn = true;
      if (userIds.size) {
        const filter = buildInFilter(userIds);
        if (filter) {
          const viewParams = new URLSearchParams({ select: 'id,full_name,children_count,show_children_count' });
          viewParams.append('id', filter);
          try {
            const profileData = await supabaseRequest(
              `${supaUrl}/rest/v1/profiles_with_children?${viewParams.toString()}`,
              { headers }
            );
            profiles = Array.isArray(profileData) ? profileData : [];
            showColumn = profiles.some((row) => Object.prototype.hasOwnProperty.call(row, 'show_children_count'));
            hasChildCount = true;
          } catch (err) {
            console.warn('[anon-community] profiles_with_children fetch failed', err);
            hasChildCount = false;
            showColumn = false;
            const fallbackParams = new URLSearchParams({ select: 'id,full_name,show_children_count' });
            fallbackParams.append('id', filter);
            const profileData = await supabaseRequest(
              `${supaUrl}/rest/v1/profiles?${fallbackParams.toString()}`,
              { headers }
            );
            profiles = Array.isArray(profileData) ? profileData : [];
            showColumn = profiles.some((row) => Object.prototype.hasOwnProperty.call(row, 'show_children_count'));
      ...
      profiles.forEach((p) => {
        ...
        const rawCount = hasChildCount ? p.children_count ?? p.child_count ?? null : null;
        const parsedCount = Number.isFinite(Number(rawCount)) ? Math.max(0, Number(rawCount)) : 0;
        const showFlag =
          showColumn && Object.prototype.hasOwnProperty.call(p, 'show_children_count') ? !!p.show_children_count : false;
        authors[key] = {
          full_name: p.full_name || '',
          children_count: parsedCount,
          child_count: parsedCount,
    ...
    if (action === 'recent-replies') {
      ...
      if (authorIds.length) {
        ...
        if (authorFilter) {
          const viewParams = new URLSearchParams({ select: 'id,full_name,children_count,show_children_count' });
          viewParams.append('id', authorFilter);
          let authorRows = [];
          let showColumn = true;
          let hasChildCount = true;
          try {
            ...
            showColumn = authorRows.some((row) => Object.prototype.hasOwnProperty.call(row, 'show_children_count'));
          } catch (err) {
            console.warn('[anon-community] profiles_with_children author fetch failed', err);
            showColumn = false;
            hasChildCount = false;
            const fallbackParams = new URLSearchParams({ select: 'id,full_name,show_children_count' });
            fallbackParams.append('id', authorFilter);
            const rows = await supabaseRequest(
              `${supaUrl}/rest/v1/profiles?${fallbackParams.toString()}`,
              { headers }
            );
          ...
          authorRows.forEach((p) => {
            ...
            const key = String(p.id);
            const rawCount = hasChildCount ? p.children_count ?? p.child_count ?? null : null;
            const childrenCount = Number.isFinite(Number(rawCount)) ? Math.max(0, Number(rawCount)) : 0;
            const showFlag =
              showColumn && Object.prototype.hasOwnProperty.call(p, 'show_children_count') ? !!p.show_children_count : false;
            authors[key] = {
              full_name: p.full_name || '',
