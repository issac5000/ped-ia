...
function getActionPayload(body) {
...
export async function processAnonChildrenRequest(body) {
  try {
    ...
    if (action === 'create') {
      ...
      const measurements = withChildId(buildMeasurementRecords(payload.growthMeasurements), childId);
      ...
      const teeth = withChildId(buildTeethRecords(payload.growthTeeth), childId);
      if (teeth.length) {
        await supabaseRequest(
      ...
      const sleep = withChildId(buildSleepRecords(payload.growthSleep), childId);
      if (sleep.length) {
        await supabaseRequest(
    ...
    if (action === 'log-update') {
      ...
      const contentRaw = payload.updateContent ?? payload.content ?? null;
      const isEmptyObject =
      ...
      if (aiSummary) updateRow.ai_summary = aiSummary;
      if (aiCommentaire) updateRow.ai_commentaire = aiCommentaire;
      const inserted = await supabaseRequest(
    ...
    if (action === 'add-growth') {
      ...
      const sleep = withChildId(buildSleepRecords(payload.growthSleep ?? payload.sleepEntries), childId);
      if (sleep.length) {
        await supabaseRequest(
          `${supaUrl}/rest/v1/growth_sleep`,
          {
      ...
      const teeth = withChildId(buildTeethRecords(payload.growthTeeth ?? payload.teethEntries), childId);
      if (teeth.length) {
        await supabaseRequest(
          `${supaUrl}/rest/v1/growth_teeth`,
          {
