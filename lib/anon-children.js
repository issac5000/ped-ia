...
async function clearFamilyContextAiBilan(supaUrl, serviceKey, profileId) {
  ...
  await supabaseRequest(
    `${supaUrl}/rest/v1/family_context?profile_id=eq.${encodeURIComponent(normalizedProfileId)}`,
    {
      ...
      method: 'PATCH',
      body: JSON.stringify({ ai_bilan: null }),
    }
  );
  console.log('[AI DEBUG] family_context ai_bilan cleared after child update', {
...
async function invalidateFamilyContext(supaUrl, serviceKey, profileId) {
  ...
  try {
    try {
      ...
      console.warn('[anon-children] unable to clear family_context ai_bilan', {
        profileId: normalizedProfileId,
        err,
      });
    }
