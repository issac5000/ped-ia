...
async function fetchGrowthDataForAnonPrompt(supaUrl, headers, childId, { measurementLimit = 3, teethLimit = 3 } = {}) {
  ...
  const measurementUrl = `${supaUrl}/rest/v1/child_growth_with_status?select=agemos,height_cm,weight_kg,status_weight,status_height,status_global&child_id=eq.${encodeURIComponent(childId)}&order=agemos.desc.nullslast&limit=${limitedMeasurements}`;
  const teethUrl = `${supaUrl}/rest/v1/growth_teeth?select=month,count,created_at&child_id=eq.${encodeURIComponent(childId)}&order=month.desc&order=created_at.desc&limit=${limitedTeeth}`;
...
async function callOpenAi(messages, { temperature = 0.4 } = {}) {
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: 'gpt-4.1-nano', temperature, messages })
  });
  if (!res.ok) {
...
async function resetFamilyContextState({
  supaUrl,
  serviceKey,
  profileId,
  childId = null,
  ...
  if (logGrowth) {
    console.log('[AI DEBUG] growth update received', {
      ...
      newValues: {
        height_cm: sanitizeMetric(height_cm),
        weight_kg: sanitizeMetric(weight_kg),
      },
    });
  }
  ...
  try {
    ...
    const clearedLabel = logGrowth
      ? '[AI DEBUG] family_context ai_bilan cleared after growth update'
      : '[AI DEBUG] family_context ai_bilan cleared';
    console.log(clearedLabel, { profileId: normalizedProfileId });
  } catch (err) {
    const warnLabel = logGrowth
      ? '[anon-children] unable to invalidate family_context'
      : '[anon-children] unable to clear family_context ai_bilan';
...
async function clearFamilyContextAiBilan(supaUrl, serviceKey, profileId, options = {}) {
  await resetFamilyContextState({ supaUrl, serviceKey, profileId, logGrowth: false, ...options });
}

async function invalidateFamilyContext(supaUrl, serviceKey, profileId, options = {}) {
  await resetFamilyContextState({ supaUrl, serviceKey, profileId, logGrowth: true, ...options });
}
...
export async function processAnonChildrenRequest(body) {
  try {
    ...
    if (action === 'log-update') {
      ...
      const row = Array.isArray(inserted) ? inserted[0] : inserted;
      await invalidateFamilyContext(supaUrl, serviceKey, profileId, { childId });
