...
async function fetchGrowthDataForAnonPrompt(supaUrl, headers, childId, { measurementLimit = 3, teethLimit = 3 } = {}) {
  ...
  const measurementUrl = `${supaUrl}/rest/v1/child_growth_with_status?select=agemos,height_cm,weight_kg,status_weight,status_height,status_global,created_at&child_id=eq.${encodeURIComponent(childId)}&order=agemos.desc.nullslast&limit=${limitedMeasurements}`;
