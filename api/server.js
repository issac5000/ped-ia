...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/api/like') {
    try {
      ...
      if (!replyId) {
        return send(res, 400, JSON.stringify({ error: 'replyId required' }), {
          'Content-Type': 'application/json; charset=utf-8',
        });
      }
      if (action !== 'like' && action !== 'unlike') {
        return send(res, 400, JSON.stringify({ error: 'Invalid action' }), {
          'Content-Type': 'application/json; charset=utf-8',
        });
      }
      ...
      if (bearerToken) {
        try {
          ...
          if (userRes.ok) {
            ...
            const idCandidate = userJson?.id || userJson?.user?.id || '';
            if (idCandidate) {
              actorId = String(idCandidate).trim();
            }
          }
        } catch (err) {
          console.warn('[api/like] auth user fetch failed', err);
        }
      }

      if (!actorId) {
        const codeRaw = body?.code || body?.code_unique || req.headers['x-anon-code'];
        ...
        if (code) {
          try {
            ...
            if (err?.status && err.status !== 404) {
              console.warn('[api/like] anon profile lookup failed', err);
            }
          }
        }
      }
      ...
      if (action === 'like') {
        const likePayload = JSON.stringify({ reply_id: replyId, user_id: actorId });
        const likeRes = await fetch(`${supaUrl}/rest/v1/forum_reply_likes`, {
          method: 'POST',
          headers: { ...baseHeaders, 'Content-Type': 'application/json', Prefer: 'return=minimal' },
          body: likePayload,
        });
        if (!likeRes.ok && likeRes.status !== 409) {
          const text = await likeRes.text().catch(() => '');
          return send(res, 500, JSON.stringify({ error: 'Like failed', details: text }), {
            'Content-Type': 'application/json; charset=utf-8',
          });
        }
      } else {
        const unlikeUrl = `${supaUrl}/rest/v1/forum_reply_likes?reply_id=eq.${encodeURIComponent(replyId)}&user_id=eq.${encodeURIComponent(actorId)}`;
        const unlikeRes = await fetch(unlikeUrl, {
          method: 'DELETE',
          headers: { ...baseHeaders, Prefer: 'return=minimal' },
        });
        if (!unlikeRes.ok) {
          const text = await unlikeRes.text().catch(() => '');
          return send(res, 500, JSON.stringify({ error: 'Unlike failed', details: text }), {
            'Content-Type': 'application/json; charset=utf-8',
          });
        }
      }

      const countUrl = `${supaUrl}/rest/v1/forum_reply_likes?reply_id=eq.${encodeURIComponent(replyId)}&select=reply_id`;
      const countRes = await fetch(countUrl, {
        headers: { ...baseHeaders, Prefer: 'count=exact' },
      });
      if (!countRes.ok) {
        const text = await countRes.text().catch(() => '');
        return send(res, 500, JSON.stringify({ error: 'Count failed', details: text }), {
          'Content-Type': 'application/json; charset=utf-8',
        });
      }
      const raw = await countRes.text().catch(() => '');
      let parsedCount = 0;
      if (raw) {
        try {
          const arr = JSON.parse(raw);
          if (Array.isArray(arr)) parsedCount = arr.length;
        } catch {}
      }
      const range = countRes.headers.get('content-range');
      if (range) {
        const totalPart = range.split('/').pop();
        const numeric = Number(totalPart);
        if (Number.isFinite(numeric) && numeric >= 0) parsedCount = numeric;
      }

      return send(res, 200, JSON.stringify({ count: parsedCount, liked: action === 'like' }), {
        'Content-Type': 'application/json; charset=utf-8',
      });
    } catch (err) {
      console.error('[api/like] handler error', err);
      return send(res, 500, JSON.stringify({ error: 'Server error', details: String(err?.message || err) }), {
        'Content-Type': 'application/json; charset=utf-8',
      });
    }
  }
